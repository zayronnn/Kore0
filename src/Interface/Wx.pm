...
sub createMenuBar {
	...
	#$menu->Append($self->{aliasMenu}, T('&xRaysGT'));

	#xRaysGT menu
	my $xRaysopMenu = new Wx::Menu;
	$menu->Append($xRaysopMenu, T('x&RaysGT'));
	$self->{mManual} = $self->addMenu($xRaysopMenu, T('Reload config.txt'), sub {Commands::run("reload config.txt")}, T('Reload config.txt'));
	$self->{mManual} = $self->addMenu($xRaysopMenu, T('AI Clear'), sub {Commands::run("ai clear")}, T('Clear AI'));
	$xRaysopMenu->AppendSeparator;
	# $self->{mManual} = $self->addMenu($xRaysopMenu, T('Turn on killSteal'), sub {Commands::run("conf killSteal 1")}, T('kill-Steal On'));
	# $self->{mManual} = $self->addMenu($xRaysopMenu, T('Turn off killSteal'), sub {Commands::run("conf killSteal 0")}, T('kill-Steal Off'));
	# $xRaysopMenu->AppendSeparator;	
	# $self->{mManual} = $self->addMenu($xRaysopMenu, T('Turn on teleport search'), sub {Commands::run("teleport_search 1")}, T('Turn on teleport search'));
	# $self->{mManual} = $self->addMenu($xRaysopMenu, T('Turn off teleport search'), sub {Commands::run("teleport_search 0")}, T('Turn off teleport search'));
	# $xRaysopMenu->AppendSeparator;
	$self->{mManual} = $self->addMenu($xRaysopMenu, T('reload macro'), sub {Commands::run("reload macro")}, T('reload macro'));
	$self->{mManual} = $self->addMenu($xRaysopMenu, T('reload portals'), sub {Commands::run("reload portals")}, T('reload portals'));
	$xRaysopMenu->AppendSeparator;
